...
Last updated: 2026-06-21 00:58 CEST / 2026-06-20 22:58 UTC
...
- [x] Source repo exists and is current. Evidence: `/home/dibanez/k8s/libreplay` on `main`, head `be5555f feat: add mobile pwa readiness` before the follow-up docs-only audit update.
- [x] GitOps runtime manifest is deployed and pushed. Evidence: `/home/dibanez/k8s/k8s-libreplay-pocharlies` on `deploy/prod`; current runtime manifest commit `ccfa39c fix: deploy mobile pwa readiness` is pushed and GitOps CI run `27886232754` completed `success`.
- [x] LAN runtime is healthy. Evidence: Argo `libreplay` is `Synced|Healthy|ccfa39c37a7e7b1fd280a8436a062c06e15e7a69|k8s`; runtime web image `sha-be5555f9dc0f@sha256:97308d307d17ba51ad0e9b1b39246b4621572d92fa0762199350407402f2f8e4`, worker image `worker-sha-be5555f9dc0f@sha256:b916acfa1298d4bec0910c704485c527f7ab15aac13b96dc706d67207d5145b2`; `/api/health/deps` returned postgres, redis, queues, minio and meilisearch `ok:true`.
...
- [x] Full LAN E2E is green. Evidence: `NODE_TLS_REJECT_UNAUTHORIZED=0 BASE_URL=https://libreplay.lan.e-dani.com PWRETRIES=0 pnpm --filter @libreplay/web exec playwright test --reporter=line` -> `97 passed (2.3m)`.
