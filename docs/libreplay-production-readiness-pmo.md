...
Last updated: 2026-06-21 03:03 CEST / 2026-06-21 01:03 UTC
...
- [x] Source repo exists and is current. Evidence: `/home/dibanez/k8s/libreplay` on `main`, head `707a837 test(auth): align staging env fixtures`.
- [x] GitOps runtime manifest is deployed and pushed. Evidence: `/home/dibanez/k8s/k8s-libreplay-pocharlies` on `deploy/prod`; current runtime manifest commit `9848ef4 feat(media): add minio lifecycle policy gate` is pushed and GitOps CI run `27888429666` completed `success`.
...
- [x] LAN runtime is healthy. Evidence: Argo `libreplay` is `Synced|Healthy|9848ef458cca0c6c2c0645cea4bf0e292b8a9346|k8s`; runtime web image `sha-707a8375192c@sha256:8b47b9d204b84b86d3ede7fde6bb9ea21385ac788919f8d2a433f007a47f2366`, worker image `worker-sha-707a8375192c@sha256:e293049e21eb638a411438bef1d4677075ed7cebbf80aa224732b51a1ea16974`; `/api/health/deps` returned postgres, redis, queues, minio and meilisearch `ok:true`.
...
- [blocked] Runtime root-cause/SLO cleanup remains incomplete. Evidence: metrics plus baseline dashboard/alert rules now exist and are loaded, but production still needs SLO definitions, structured logs, error tracking, uptime synthetic checks, alert fire/route rehearsal and follow-up root-cause analysis if `P1001` or `ReadableStream is already closed` recurs under load.
...
- [x] DB-backed media retention state and dry-run planner are implemented. Evidence: source commit `50e1d14` adds `MediaRetentionClass`, `MediaPurgeState`, delete/purge/legal-hold fields, migration `20260621003500_media_retention_lifecycle`, default-dry-run `media:retention` CLI, relation blockers for posts/albums/messages/covers/PPV/paid content/verification/consent/reports/moderation, and tests; source CI `27888886986` and Release Image `27889021579` passed.
...
- [blocked] Production media delivery is not complete. Evidence: one 480p MP4/HLS rendition, bucket noncurrent lifecycle and DB-backed retention planner are validated, but production still lacks a bitrate ladder, CDN/signed delivery strategy, async progress UI, version-aware physical purge/orphan cleanup, failure replay/dead-letter runbook, real CSAM/media moderation providers and multi-replica worker scaling.
...
- [ ] Video scalability exists. Missing: bitrate ladder beyond 480p, CDN strategy, async job progress UI, physical purge/orphan cleanup, failure replay and production worker scaling.
...
- [ ] Observability is production-grade. Missing: structured logs, error tracking, SLOs, uptime synthetic checks, alert fire/route rehearsal and recurrence analysis for prior `P1001`/stream-close residuals; baseline dashboards and alert rules are now present.
...
- [ ] Complete production media delivery/lifecycle pipeline.
  Evidence required: bitrate ladder beyond 480p, CDN/signed delivery strategy, async job progress, version-aware physical purge/orphan cleanup, production worker scaling and failure replay/dead-letter plan.
...
- [ ] Add observability and alerting.
  Evidence required: baseline dashboards and alerts are present; remaining evidence required is structured logs, error tracking, SLOs, uptime synthetic checks and alert fire/route rehearsal.
