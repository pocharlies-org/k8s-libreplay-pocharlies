...
- [blocked] Add staging real-auth mailbox secret.
  Evidence required: GitHub environment secret `PW_STAGING_AUTH_EMAIL` exists and points to a controlled plus-addressable or catch-all staging mailbox; current strict preflight reports it missing.
...
- [ ] Ensure Google/Facebook OAuth credentials plus `OAUTH_TOKEN_ENC_KEY`, SMTP secrets and `PW_STAGING_AUTH_EMAIL` exist without exposing secret values in logs/docs.
