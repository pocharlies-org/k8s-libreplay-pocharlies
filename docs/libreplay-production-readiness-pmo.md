...
- [x] Stripe PPV sandbox-only gate exists without live activation. Evidence: source commit `f3b9301`, source CI run `27884105851`, config rejects Stripe in production and requires sandbox/test-key/ref gates, PPV checkout reuses pending local transactions and grants only from signed paid Stripe webhooks, independent verifier PASS, and GitOps staging/production remain `PAYMENT_PROVIDER=disabled` with no Stripe env/secrets.
...
- [x] Stripe PPV sandbox skeleton exists without live activation. Evidence: source commit `f3b9301` adds Stripe SDK usage behind sandbox/test-key/ref gates, signed Checkout Session webhook handling and PPV-only pending transaction idempotency; GitOps staging/production stay disabled.
...
- [x] Add sandbox-only Stripe PPV checkout/webhook gate without production activation.
  Evidence: source commit `f3b9301`, source CI `27884105851`, production rejects Stripe, staging requires sandbox/test-key/ref gates, PPV grants only after signed paid webhook, and GitOps remains `PAYMENT_PROVIDER=disabled`.
