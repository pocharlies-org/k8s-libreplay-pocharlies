...
require_pattern "$manifest" 'udp_port:[[:space:]]*50000' 'LiveKit RTC uses the single UDP mux port exposed by its Service'
reject_pattern "$manifest" 'port_range_(start|end):' 'LiveKit RTC does not advertise an unexposed UDP range'
